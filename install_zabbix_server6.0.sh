...
# zabbix 6.0 LTS for RHEL\CentOS\Oracle\Rocky 8,9
...
#MYSQL_HOST=172.16.128.25
#MYSQL_ZABBIX_USER="zabbix@'172.16.128.%'"
...
install_mysql() {
    ...
    if [ $ID = "centos" -o $ID = "rocky" ]; then
        if [ ${VERSION_ID} == "8" -o ${VERSION_ID} == "9" ]; then
...
install_zabbix() {
    if [ $ID = "centos" -o $ID = "rocky" ]; then
        if [ ${VERSION_ID} == "8" -o ${VERSION_ID} == "9" ]; then
            rpm -Uvh https://${URL}/zabbix/${ZABBIX_VER}/rhel/${VERSION_ID}/x86_64/zabbix-release-${ZABBIX_VER}-4.el${VERSION_ID}.noarch.rpm
            if [ $? -eq 0 ]; then
                color "YUM仓库准备完成" 0
            else
        ...
        wget https://${URL}/zabbix/${ZABBIX_VER}/ubuntu/pool/main/z/zabbix-release/zabbix-release_${ZABBIX_VER}-3+ubuntu${VERSION_ID}_all.deb
        if [ $? -eq 0 ]; then
            color "APT仓库准备完成" 0
        else
        ...
        dpkg -i zabbix-release_${ZABBIX_VER}-3+ubuntu${VERSION_ID}_all.deb
